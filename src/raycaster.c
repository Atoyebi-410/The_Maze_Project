...
/**
 * raycaster - this casts rays and renders walls, floor, and ceiling
...
void raycaster(int *maze, bool textured)
{
	...
	int x; /* this is ray counter */

	/* cast ray x for every column w */
	for (x = 0; x < SCREEN_WIDTH; x++)
	{
		/* this calculate ray position and direction */
		...
		/* this is grid position on maze */
		map.x = (int)(rayPos.x);
		map.y = (int)(rayPos.y);

		/* this measure distance to next X or Y intersection */
		distToNext.x = sqrt(1 + (pow(rayDir.y, 2)) / pow(rayDir.x, 2));
		distToNext.y = sqrt(1 + (pow(rayDir.x, 2)) / pow(rayDir.y, 2));

		/* this calculates step and distance from pos to first X or Y */
		if (rayDir.x < 0)
		{
			step.x = -1;
		...
		/* moves to next maze square in X or Y dir until wall is hit */
		hit = 0;
		while (hit == 0)
		{
