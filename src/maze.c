...
/**
 * main - this renders maze
 * @argc: this is number of arguments
 * @argv: this is pointer to array of arguments
 * Return: 0 on success, 1 on failure
 */
int main(int argc, char *argv[])
{
	int *maze; /* this is 2D array defining maze map */
	char *mapName;
	bool textured;

	/* this is the initial values for global variables */
	pos.x = 1;
	pos.y = 12;
	dir.x = 1;
	dir.y = -0.66;
	...
	/* this loops until user exits by ESC or closing window */
	while (!quit())
	{
		if (!textured)
			renderBGFlat();

		/* this draw walls, textured floor, and textured ceiling */
	...
	/* this close SDL, renderer, and window */
	closeSDL();
	free(maze);
	return (0);
}
